<template>
  <el-card class="mycard">
    <!-- 面包屑导航 -->
    <Mybread :navone="'商品管理'" :navtwo="'商品列表'"/>
    <!-- 消息提示框 -->
  ...
  data() {
    return {
      ...
      cateObj: {
        ...
        expandTrigger: "hover" // 触发下一层级选项的方式
      },
      cateSelect: [], // 当前级联框选中的数据
      manyparams: [], // 静态参数（商品属性）
      onlyparams: [], // 动态参数（商品参数）
      check: true,
      fileList: [], // 文件上传列表的集合
      uploadHead: {
        Authorization: localStorage.getItem('token')
      }, // 上传的请求头
      dialogImg: false, // 控制图片预览时的对话框的显示与隐藏
      content: '我是富文本框中的默认值'
    };
  },
  methods: {
    ...
    remove(file, fileList){
        // 通过 file 中的属性去 删除 fileList 中的数据
    ...
    preview(file){
        // console.log('dianle')
        this.dialogImg = true
        // 是通过 dome 去操作 vue 的页面 （页面不会有响应式的效果）
